...
class Rule(Comparable, BaseModel):
    ...
    def get_contrapositives(self) -> Iterator[Rule]:
        ...
        for input_factor in self.inputs:
            ...
            if isinstance(next_input, AbsenceOf):
                next_input = next_input.absent
            else:
                next_input = AbsenceOf(absent=next_input)
            next_output = deepcopy(self.outputs[0])
            if not isinstance(next_output, AbsenceOf):
                next_output = AbsenceOf(absent=next_output)
