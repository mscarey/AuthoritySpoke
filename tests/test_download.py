...
class TestDownload:
    def test_download_opinion_by_id(self, tmp_path):
        ...
        Opinion.cap_download(cap_id=4066790, filename=to_file, directory=tmp_path, to_dict=True)
    ...
    def test_download_opinion_by_cite(self, tmp_path):
        ...
        Opinion.cap_download(cite="49 F.3d 807", filename=to_file, directory=tmp_path, to_dict=True)
        new_file = tmp_path / to_file
        with open(new_file, "r") as read_file:
            case = json.load(read_file)
